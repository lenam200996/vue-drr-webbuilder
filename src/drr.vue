<template>
  ...
  >
    <div v-if="showLineXL" class="xLeft"></div>
    <div v-if="showLineXR" class="xRight"></div>
    <div v-if="showLineYT" class="yTop"></div>
    <div v-if="showLineYB" class="yBottom"></div>
    <div v-if="showXCenter" class="xCenter"></div>
    <div v-if="showYCenter" class="yCenter"></div>
    <slot></slot>
    <div v-if="showPosition" class="show-position">
      <div class="row1">
        <span>x: {{parseFloat(style.left).toFixed(1)}}</span>
        <span>y: {{parseFloat(style.top).toFixed(1)}}</span>
      </div>
      <div class="row2">
        <span>w: {{parseFloat(style.width).toFixed(1)}}</span>
        <span>h: {{parseFloat(style.height).toFixed(1)}}</span>
      </div>
    </div>
    <vue-context ref="menuContext">
      <li @click="contextMenu">Set as Element</li>
    </vue-context>
    <div
...
const moduleElement = createNamespacedHelpers("element");
...
export default {
  ...
  watch: {
    active(val) {
      if (val) {
        this.$emit("select");
        this.index == "body" ? this.updateGridAction() : null;
        setTimeout(() => {
    ...
    rect(val) {
      var xl = this.cx - this.width / 2;
      var xr = xl + this.width;
      var yt = this.cy - this.height / 2;
      var yb = yt + this.height;
      var xc = parseInt(xl + this.width / 2);
      var yc = parseInt(yt + this.height / 2);
      this.showLineXL = this.grid.xl.includes(xl) && this.active;
      this.showLineXR = this.grid.xr.includes(xr) && this.active;
      this.showLineYT = this.grid.yt.includes(yt) && this.active;
      this.showLineYB = this.grid.yb.includes(yb) && this.active;
      this.showXCenter = this.grid.xc.includes(xc) && this.active;
      this.showYCenter = this.grid.yc.includes(yc) && this.active;
    }
  },
  ...
  methods: {
    openContext(ev) {
      bus.$emit("closeContext", true);
      setTimeout(() => {
        this.$refs.menuContext.open(ev.clientY, ev.clientX);
      }, 50);
    },
    contextMenu() {
      ...
      bus.$emit("openModal", {
        type: this.type,
        id: this.$store.getters["status/getElementIdSelected"]
      });
    },
    ...moduleElement.mapActions(["updateGridAction"]),
    dblclick(e) {
    ...
    move(ev) {
      ...
      var incX = 0;
      var incY = 0;
      this.grid.xl.forEach(value => {
        if (Math.abs(value - xl) < 5) {
          incX = value - xl;
        }
      });
      this.grid.xr.forEach(value => {
        if (Math.abs(value - xr) < 5) {
          incX = value - xr;
        }
      });
      this.grid.yt.forEach(value => {
        if (Math.abs(value - yt) < 5) {
          incY = value - yt;
        }
      });
      this.grid.yb.forEach(value => {
        if (Math.abs(value - yb) < 5) {
          incY = value - yb;
        }
      });
      this.grid.xc.forEach(value => {
        if (Math.abs(value - xc) < 5) {
          incX = value - xc;
        }
      });
      this.grid.yc.forEach(value => {
        if (Math.abs(value - yc) < 5) {
          incY = value - yc;
        }
      });
      this.cx += incX;
      this.cy += incY;
      // if(this.grid.xr.includes(xr - 5)){
      //   console.log('ok')
      //   this.cx += 5
      // }
      // if(this.grid.xr.includes(xr + 5)){
    ...
    up(ev) {
      ...
      this.showLineXL = false;
      this.showLineXR = false;
      this.showLineYT = false;
      this.showLineYB = false;
